...
def preprocess(image, shape=[64, 64]):
    ...
    print("AAAAA --- {}".format(image.shape))
    image = tf.image.resize(image, shape)
    print("BBBBB --- {}".format(image.shape))
    image = tf.reshape(image, shape + [3])
    print("CCCCC --- {}".format(image.shape))
...
def build_model(input_shape=[64, 64]):
    ...
    # model.add(tf.keras.layers.Input([1280], batch_size=1, dtype=tf.float32))
